...
def main():
    ...
    csv_handler = CSVHandler(csv_file_path, SOURCE_OF_TRUTH_COLUMNS)

    # Step 1: Read database data
    db_data = db_handler.read_database_data()

    # Step 2: Update the CSV from the DB for source-of-truth columns (case-insensitive matching)
    csv_handler.update_csv(db_data)

    # Step 3: Read the now-updated CSV data
    csv_data = csv_handler.read_csv_data()
    ...
    print("Synchronization between CSV and database completed successfully.")
